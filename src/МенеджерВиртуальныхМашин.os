...
Процедура ЗагрузитьИнформациюВМ()
	...
	Для каждого ИмяВМ Из МассивВМ Цикл
		ВМ = Новый ВиртуальнаяМашина(СокрЛП(ИмяВМ)); 
...
Функция Создать(Знач ИмяВМ) Экспорт
	...
	Если ПустаяСтрока(Результат) или  СтрНайти(Результат,"Работает нормально") = 0 Тогда
		Лог.Ошибка("Не удалось создать ВМ!");
		ВызватьИсключение ИмяВМ+": Не удалось создать ВМ!";
	КонецЕсли; 
...
Функция Удалить(Знач ИмяВМ) Экспорт
	...
	Если НЕ ПустаяСтрока(Результат) и НЕ Результат = Истина Тогда
