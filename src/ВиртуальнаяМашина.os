...
Функция Выгрузить( Знач ПутьБэкапа ) Экспорт
	...
	тзДиски = ПолучитьДискиПодробно("DiskNumber,ControllerLocation,ControllerNumber,ControllerType,PoolName");
	//

	СтруктураПоиска = Новый Структура("PoolName", "");
	ТранзитныеДиски = тзДиски.НайтиСтроки(СтруктураПоиска);
	Если ТранзитныеДиски.Количество() > 0 Тогда

		Если ПолучитьСостояние() = "Running" Тогда
			НуженЗапуск = Истина;
			Ответ = Остановить("-Force");
			Если Ответ = Ложь Тогда
				Лог.Ошибка("Не удалось остановить " + ВиртуальнаяМашина + " с транзитными дисками!");
				Лог.Ошибка("Сбой выгрузки " + ВиртуальнаяМашина + "!");
				Возврат Ложь;
			КонецЕсли;
		КонецЕсли;

//		Для каждого Диск Из ТранзитныеДиски Цикл
//			Если Не ОтключитьДиск(Диск.ControllerType, Диск.ControllerLocation, Диск.ControllerNumber) Тогда
//				Лог.Ошибка("Не удалось отключить транзитный диск на виртуальной машине " + ВиртуальнаяМашина + "!");
//				Лог.Ошибка("Сбой выгрузки " + ВиртуальнаяМашина + "!");
//				ЕстьОшибка = Истина;
//				Прервать;	
//			КонецЕсли; 
...
//		КонецЕсли; 
	...
	Если ТранзитныеДиски.Количество() > 0 Тогда
//		Для каждого Диск Из ТранзитныеДиски Цикл
//			Если Не ПолключитьДиск(Диск.DiskNumber, Диск.ControllerType, Диск.ControllerLocation, Диск.ControllerNumber) Тогда
//				Лог.Ошибка("Не удалось подключить транзитный диск на виртуальной машине " + ВиртуальнаяМашина + "!");
//				Лог.Ошибка("Сбой выгрузки " + ВиртуальнаяМашина + "!");
//				ЕстьОшибка = Истина;
//			КонецЕсли; 	
//		КонецЦикла;
		Если Не ЕстьОшибка и НуженЗапуск Тогда
			ЕстьОшибки = Не Запустить();
		КонецЕсли;
	КонецЕсли; 
...
Функция ВыгрузитьВМ( Знач ПутьБэкапа )
	...
	Вывод = ВыполнитьКоманду(ТекстКоманды, Истина);

	Если НЕ ПустаяСтрока(Вывод) и СтрНайти(Вывод, " 0x5 occurred while reading the console output buffer.") = 0 Тогда
		Лог.Ошибка(Вывод);
		Возврат Ложь;
	КонецЕсли;
...
Функция Остановить(Знач Метод = "-TurnOff") Экспорт
	...
	ИначеЕсли НЕ ПустаяСтрока(Вывод) Тогда
		Лог.Ошибка(Вывод);
		Возврат Ложь;
	КонецЕсли;
...
Функция ПолучитьДиски() Экспорт
	...
	ТекстКоманды = "Get-VM -VMName " + ВиртуальнаяМашина + " | Get-VMHardDiskDrive | Where-Object Path -like '*.vhdx' | select Path ";
	Результат = ВыполнитьКоманду(ТекстКоманды, Истина);
...
// "VMName,Name,Path,DiskNumber,ControllerLocation,ControllerNumber,ControllerType,PoolName";
Функция ПолучитьДискиПодробно(Знач СтрокаКолонки = "*") Экспорт

	ТекстКоманды = "get-vm -Name " + ВиртуальнаяМашина + " |Get-VMHardDiskDrive | Format-List -Property " + СтрокаКолонки;
...
КонецФункции //ПолучитьДискиПодробно
