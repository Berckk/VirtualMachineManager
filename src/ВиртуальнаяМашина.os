...
//
// Параметры:
//  ПутьБэкапа   - Строка - Каталог куда будет выгружаться виртуальные машины. Должен быть пустым.
//
...
Функция Выгрузить( Знач ПутьБэкапа ) Экспорт
	...
	Ожидаем.Что(ФС.КаталогСуществует(ПутьКДискам), ВиртуальнаяМашина +": Произошел сбой выгрузки. Каталог """+ПутьКДискам+""" пустой.").ЕстьИстина();
...
Функция ВыгрузитьВМ( Знач ПутьБэкапа )
	...
	Пока
		...
		Лог.Отладка(ВиртуальнаяМашина + ":" + Статус);
	КонецЦикла;

	Если Статус <> "Работает нормально" Тогда
		Лог.Ошибка(ВиртуальнаяМашина + ": Вышло время ожидания выгрузки виртуальной машины");
		Возврат Ложь;		
	КонецЕсли;
...
// Метод - Значение Force, TurnOff и Save
...
Функция Остановить(Знач Метод = "-TurnOff") Экспорт
	...
	Пока
		...
		Приостановить(60*ОднаСекунда);
		ОбновитьСостояние();
		Лог.Отладка(ВиртуальнаяМашина + ":" + Состояние);
	КонецЦикла;
...
//
// Параметры:
//  Путь - 'd:\test.vhdx' Путь к файлу диска
//               Тип диска определяется по расширению файла vhdx
//  Размер  - "1GB"
// ДопПараметры -  -BlockSizeBytes 128MB -LogicalSectorSize 4KB -Dynamic -Differencing
//
// Возвращаемое значение:
//   Истина в случае успеха.
//
Функция ДобавитьДиск(Знач Путь, Знач Размер, Знач ДопПараметры =  "-Dynamic") Экспорт
	Ожидаем.Что(Путь, "Передан аргумент неверного типа").ИмеетТип("Строка");
	Ожидаем.Что(Размер, "Передан аргумент неверного типа").ИмеетТип("Строка");
	Ожидаем.Что(ДопПараметры, "Передан аргумент неверного типа").ИмеетТип("Строка");

	ТекстКоманды =  " New-VHD -Path '" + Путь + "' -SizeBytes " + Размер + " " + ДопПараметры; 	
    
	Результат = ВыполнитьКоманду(ТекстКоманды, Истина);
	Если СтрНайти(Результат, "DiskIdentifier") = 0  и  СтрНайти(Результат, " 0x5 occurred while reading the console output buffer.") = 0 Тогда
		Лог.Ошибка(Результат);
		ВызватьИсключение "Не удалось создать диск";		
	КонецЕсли;

	ТекстКоманды = " Add-VMHardDiskDrive -VMName " + ВиртуальнаяМашина + " -Path " + Путь;
...
Функция УстановитьВиртуальнуюМашину(Знач ИмяВМ)
	...
	Если МассивВМ.Количество() = 1 Тогда
		ВиртуальнаяМашина = СокрЛП(ИмяВМ);
		Имя = ИмяВМ;
		Состояние = МассивВМ[0];
		Работает = ?(Состояние = "Running", Истина, Ложь);
		Возврат Истина;
	ИначеЕсли МассивВМ.Количество() > 1 Тогда
		ВызватьИсключение "Имя виртуальной машины не уникально.";
	Иначе
