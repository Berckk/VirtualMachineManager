...
Функция ВыгрузитьВМ( Знач ПутьБэкапа )
	...
	Пока (Статус = "Экспорт виртуальной машины"
...
Функция ПолучитьДиски() Экспорт
	...
	Для ИИ = времМассив.Количество() - 1 по 0 Цикл
		Если Прав(времМассив.Получить(ИИ), 5) = ".vhdx" Тогда
			Продолжить;
		Иначе
			времМассив.Удалить(ИИ);
		КонецЕсли;
